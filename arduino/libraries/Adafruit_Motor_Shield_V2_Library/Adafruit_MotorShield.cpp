...
void Adafruit_StepperMotor::step(uint16_t steps, uint8_t dir,  uint8_t style, uint8_t ocr_val) {
  ...
  while (steps--) {
    ...
    ret = onestep(dir, style, ocr_val);
    delayMicroseconds(uspers);
    yield(); // required for ESP8266
  }
}

uint8_t Adafruit_StepperMotor::onestep(uint8_t dir, uint8_t style, uint8_t ocr_val) {
  uint8_t a, b, c, d;
  uint8_t ocrb, ocra;
